#' @title Core cover set
#' @description Draws the core-cover of a 2, 3 or 4 person compromise admissible TU game.
...
#' @param col Color.
#' @return  If set=TRUE, it displays the core-cover of the game defined by \eqn{v} inside the imputation set.
#' If set=FALSE, it displays the core-cover inside the last graph.
...
#' @seealso \link{CoreCoverVertices}, \link{CoreSet}, \link{EpsilonCoreSet}, \link{HarsanyiSet}, \link{ImputationSet}, \link{LeastCoreSet}, \link{UtopiaPayoffs}, \link{WeberSet}
#' @export

CoreCoverSet=function(v,set=TRUE,col=NULL){
  Characteristicdata(v)
  n=Characteristicdata(v)$n
  if(n>4)
    stop('The core cover set can only be represented if n<=4',call.=F)
  nC=Characteristicdata(v)$nC
  if(n==2){
    if(v[3]==(v[1]+v[2])){
      vi=c(v[1],v[2])
      return(paste('The core cover set is a single-point: (',toString(vi),')',sep=""))
    }
    if(v[3]<(v[1]+v[2])){
      return(paste('The core cover set is empty',sep=""))
    }
    m=UtopiaPayoffs(v)$m
    ...
    V=CoreCoverVertices(v)

    if(dim(V)[1]==0)
      stop('The Core cover set is empty',call.=F)
    m=UtopiaPayoffs(v)$m
    ...
    plot3d(vertices2,type="l",
           ...
           col="black",
           lwd=1,
           main=paste("Core cover set for ","v=(",toString(v),")",sep=""),
           box=F)}
